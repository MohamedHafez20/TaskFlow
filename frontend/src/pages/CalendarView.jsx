  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3.5 mb-8 px-2">
        ...
        <div>
          <h1 className="text-xl font-black text-ink tracking-wide">Calendar Schedule</h1>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.5fr_0.9fr] gap-6 items-start">
        ...
        <div className="rounded-3xl bg-card border border-hair p-5 md:p-7 shadow-xl">
          ...
          <div className="flex items-center justify-between mb-8 pb-4 border-b border-hair">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={prevMonth}
                className="h-8 w-8 rounded-xl bg-hair border border-hair flex items-center justify-center text-muted hover:text-ink hover:bg-hair transition-all"
              >
                ...
                <button
                  ...
                  className={`aspect-square rounded-2xl font-mono text-sm font-bold flex flex-col items-center justify-between p-2.5 relative transition-all duration-200 select-none ${
                    day === null
                      ...
                      : total > 0
                      ? "bg-card2 border border-purple-500/20 text-purple-300 hover:bg-card2 hover:border-purple-500/40"
                      : "bg-card2 border border-hair text-muted hover:bg-hair hover:text-ink"
                  }`}
                >
                  {day !== null && (
                    <>
                      <span className={`self-start ${isSelected ? "text-ink" : "text-sub"}`}>
        ...
        <div className="rounded-3xl bg-card border border-hair p-5 md:p-6 lg:sticky lg:top-6 shadow-xl flex flex-col min-h-[420px]">
          <div className="border-b border-hair pb-4 mb-4">
            ...
            <h3 className="text-sm font-black text-ink mt-1 tracking-wide font-mono">
              {selectedDate ? (
            ...
            <AnimatePresence mode="wait">
              {selectedDate ? (
                tasksForSelectedDate.length > 0 ? (
                      ...
                      <div
                        ...
                        className={`p-3.5 rounded-2xl border transition-all duration-300 flex flex-col gap-1.5 ${
                          task.completed
                            ...
                            : "bg-card2 border-hair"
                        }`}
                      >
                        <p className={`text-xs font-bold tracking-wide ${task.completed ? "line-through text-muted" : "text-ink"}`}>
                          {task.title}
                        </p>
                        <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-wider text-muted">
                  ...
                  >
                    <p className="text-xs font-bold text-muted">✨ Blank Space On Dashboard</p>
                    <p className="text-[10px] text-faint font-medium mt-1.5 max-w-[180px]">
                ...
                <motion.div
                  ...
                  className="h-full flex flex-col items-center justify-center text-center py-16"
                >
                  <p className="text-xs font-bold text-muted">👈 Timeline Navigation</p>
                  <p className="text-[10px] text-faint mt-1">Select a slot to fetch daily tasks</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
